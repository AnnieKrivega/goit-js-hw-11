...
function forEachCard({webformatURL, largeImageURL,tags,likes,views,comments,downloads}){
    ...
    return `<div class="photo-card"><img class="card-img" 
    src="${webformatURL}" 
    alt="${tags}" loading="lazy" />
    <div class="info"><p class="info-item"><b>Likes: ${likes}</b></p><p 
    class="info-item"><b>Views: ${views}</b></p><p 
    class="info-item"><b>Comments: ${comments}</b></p><p 
    class="info-item"><b>Downloads: ${downloads}</b></p></div></div>`
    ...
    function onSubmit(evt){
        ...
        clearHtml();
        imageApi.query = refs.formEl.searchQuery.value;
        if(imageApi.query===""){
            refs.loadMoreEl.classList.add('is-hidden')
            return
        }
